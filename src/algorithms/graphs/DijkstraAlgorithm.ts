...
/**
 * Performs Dijkstra's Algorithm on a graph; giving the path from source to destination with the smallest cost
...
function DijkstraAlgorithm(
  ...
  const verticesToVisit = new PriorityQueueV2<Vertex>(
    (a, b) => graph[currentVertex][b] - graph[currentVertex][a]
  );

  const previousVertices: { [from: string]: Vertex } = {};
