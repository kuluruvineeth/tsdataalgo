...
class Counter<T> extends Map<T | string, number> {
   ...
   * @returns {number} The count associated with the key.
   */
  get(key: T): number {
  ...
  /**
   * Returns the key repeated by their coounts.
   * @returns {Array<T>} An array of keys.
   */
  elements(): Array<T> {
   ...
   * @returns {Array<T, number]>} An array of [key,count] pairs
   */
