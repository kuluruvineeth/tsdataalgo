 ...
 * It may not be all-encompassing, but its better than none.
...
class BloomFilter<T> {
   ...
   *
   * @param {number} size The size of the bloom filter to initialize as.
   ...
   * @returns {boolean} Whether the item is probably in the set or not.
   *    If a false is returned, the item is surely not in the set.
   */
  contains(item: T): boolean {
    // check if every bit at the location indicated by the hash funnction returns true
   ...
   * @returns {number} Size of bloom filter.
   */
  get size(): number {
    return this._size;
  }
