 ...
 *
 * This find operation uses path compression
 ...
 * - find: Determine which subset a particular element is in - O(alphs(n))
...
export class DisjointSet {
  ...
  find(index: number): number {
    if (this.head[index] != index) {
      // Uses path compression (sets an edge between the element and its head)
  ...
  join(first: number, second: number): void {
    ...
    //Join the smallest set wuth the bigger one
